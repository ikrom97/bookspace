      ...
      <div class="container login-page__container">
         <section class="login">
            ...
            <form class="login-form" action="{{route('auth.check')}}" method="POST">
               @csrf
               <h1 class="login-title">Вход</h2>
               @if (Session::get('fail'))
                  <p class="login-fail-msg">{{Session::get('fail')}}</p>
               @endif
                  <label class="login-label">
                     <input class="login-input" id="login" type="text" name="login" placeholder="Логин" value="{{old('login')}}">
                     @error('login') 
                        <p class="login-error-msg">{{$message}}</p> 
                     @enderror
                  </label>
                  <label class="login-label">
                     <input class="login-input" type="password" name="password" id="password" placeholder="Пароль">
         ...
         <section class="welcome">
            <div class="welcome__wrapper">
               <h2 class="txt-center">Добро пожаловать!</h2>
               <p class="welcome__text">
                  Чтобы оставаться на связи с нами, войдите с вашей личной информацией
               </p>
            </div>
